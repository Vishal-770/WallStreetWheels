import { motion } from "framer-motion";
import { NavLink } from "react-router-dom";

const Footer = () => {
  const footerVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: {
      opacity: 1,
      y: 0,
      transition: {
        duration: 0.8,
        ease: "easeOut",
      },
    },
  };

  const itemVariants = {
    hidden: { opacity: 0, y: 10 },
    visible: (i) => ({
      opacity: 1,
      y: 0,
      transition: {
        delay: i * 0.1,
        duration: 0.5,
      },
    }),
  };

  return (
    <motion.footer
      initial="hidden"
      whileInView="visible"
      viewport={{ once: true, margin: "-50px" }}
      variants={footerVariants}
      className="bg-black text-white pt-16 pb-8"
    >
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          {/* Company Info */}
          <motion.div custom={0} variants={itemVariants} className="col-span-1">
            <div className="flex items-center mb-6">
              <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mr-3">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="black"
                  className="w-6 h-6"
                >
                  <path
                    d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zM9 7.5A.75.75 0 009 9v1.5c.001.217.053.431.155.625a1.5 1.5 0 011.17.75 2.25 2.25 0 004.5 0 1.5 1.5 0 011.17-.75c.102-.194.154-.408.155-.625V9a.75.75 0 00-1.5 0v1.5h-.75a.75.75 0 000 1.5h.75v1.5a.75.75 0 001.5 0V15H15a.75.75 0 000-1.5h-.75V12a3 3 0 01-3 3 3 3 0 01-3-3V9a.75.75 0 00-.75-.75z"
                    clipRule="evenodd"
                  />
                </svg>
              </div>
              <h3 className="text-2xl font-bold tracking-tight">
                WALL STREET <span className="text-gray-300">WHEELS</span>
              </h3>
            </div>
            <p className="text-gray-400 mb-4">
              Premium luxury and exotic vehicles with unparalleled service since
              2005.
            </p>
            <div className="flex space-x-4">
              {[
                ["facebook", "https://www.facebook.com/"],
                ["twitter", "https://x.com/street_wal87754"],
                ["instagram", "https://www.instagram.com/wall_street_wheels/"],
                ["youtube", "https://www.youtube.com/@WallStreetWheels"],
              ].map((social, i) => (
                <motion.a
                  key={social[0]}
                  custom={i + 1}
                  variants={itemVariants}
                  href={social[1]}
                  className="text-gray-400 hover:text-white transition-colors duration-300"
                  whileHover={{ y: -3 }}
                >
                  <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path
                      d={
                        social[0] === "facebook"
                          ? "M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
                          : social[0] === "twitter"
                          ? "M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84"
                          : social[0] === "instagram"
                          ? "M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
                          : "M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"
                      }
                    />
                  </svg>
                </motion.a>
              ))}
            </div>
          </motion.div>

          {/* Quick Links */}
          <motion.div custom={1} variants={itemVariants} className="col-span-1">
            <h4 className="text-lg font-semibold mb-6 uppercase tracking-wider">
              Quick Links
            </h4>
            <ul className="space-y-3">
              {[
                ["Home", ""],
                ["Inventory", "cars"],
                ["Financing", "contact"],
                ["Trade-In", "contact"],
                ["Testimonials", "contact"],
              ].map((link, i) => (
                <motion.li
                  key={link[0]}
                  custom={i + 2}
                  variants={itemVariants}
                  whileHover={{ x: 5 }}
                >
                  <NavLink
                    to={`/${link[1]}`}
                    className="text-gray-400 hover:text-white transition-colors duration-300"
                  >
                    {link[0]}
                  </NavLink>
                </motion.li>
              ))}
            </ul>
          </motion.div>

          {/* Contact Info */}
          <motion.div custom={2} variants={itemVariants} className="col-span-1">
            <h4 className="text-lg font-semibold mb-6 uppercase tracking-wider">
              Contact Us
            </h4>
            <ul className="space-y-4">
              <motion.li
                custom={3}
                variants={itemVariants}
                className="flex items-start"
                whileHover={{ x: 5 }}
              >
                <svg
                  className="w-5 h-5 mr-3 mt-0.5 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
                  />
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
                  />
                </svg>
                <span className="text-gray-400">
                  Vandalur
                  <br />
                  Chennai TamilNadu
                </span>
              </motion.li>
              <motion.li
                custom={4}
                variants={itemVariants}
                className="flex items-center"
                whileHover={{ x: 5 }}
              >
                <svg
                  className="w-5 h-5 mr-3 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
                  />
                </svg>
                <a
                  href="tel:+18005551234"
                  className="text-gray-400 hover:text-white transition-colors duration-300"
                >
                  (123) 456-1234
                </a>
              </motion.li>
              <motion.li
                custom={5}
                variants={itemVariants}
                className="flex items-center"
                whileHover={{ x: 5 }}
              >
                <svg
                  className="w-5 h-5 mr-3 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                  />
                </svg>
                <a
                  href="mailto:wall.street.wheels07@gmail.com"
                  className="text-gray-400 hover:text-white transition-colors duration-300"
                >
                  wall.street.wheels07@gmail.com
                </a>
              </motion.li>
            </ul>
          </motion.div>

          {/* Business Hours */}
          <motion.div custom={3} variants={itemVariants} className="col-span-1">
            <h4 className="text-lg font-semibold mb-6 uppercase tracking-wider">
              Business Hours
            </h4>
            <ul className="space-y-3">
              {[
                { day: "Monday - Friday", hours: "9:00 AM - 7:00 PM" },
                { day: "Saturday", hours: "10:00 AM - 6:00 PM" },
                { day: "Sunday", hours: "Closed" },
                { day: "Holidays", hours: "By Appointment" },
              ].map((item, i) => (
                <motion.li
                  key={item.day}
                  custom={i + 6}
                  variants={itemVariants}
                  className="flex justify-between"
                  whileHover={{ x: 5 }}
                >
                  <span className="text-gray-400">{item.day}</span>
                  <span className="text-white">{item.hours}</span>
                </motion.li>
              ))}
            </ul>
            <motion.div
              custom={10}
              variants={itemVariants}
              className="mt-8"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              <NavLink to={"/contact"}>
                <button className="bg-white text-black px-6 py-3 rounded-md font-medium hover:bg-gray-200 transition-colors duration-300 w-full">
                  Schedule Appointment
                </button>
              </NavLink>
            </motion.div>
          </motion.div>
        </div>

        {/* Bottom Bar */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.5 }}
          className="border-t border-gray-800 pt-8 mt-8 flex flex-col md:flex-row justify-between items-center"
        >
          <p className="text-gray-500 text-sm mb-4 md:mb-0">
            &copy; {new Date().getFullYear()} Wall Street Wheels. All rights
            reserved.
          </p>
          <div className="flex space-x-6">
            <a
              href="https://policies.google.com/privacy"
              className="text-gray-500 hover:text-white text-sm transition-colors duration-300"
            >
              Privacy Policy
            </a>
            <a
              href="https://policies.google.com/privacy"
              className="text-gray-500 hover:text-white text-sm transition-colors duration-300"
            >
              Terms of Service
            </a>
            <a
              href="#"
              className="text-gray-500 hover:text-white text-sm transition-colors duration-300"
            >
              Sitemap
            </a>
          </div>
        </motion.div>
      </div>
    </motion.footer>
  );
};

export default Footer;
